...
- **Recrutadores menos técnicos podem estranhar.** "Por que não usou Devise?" vai aparecer em entrevista. Esse ADR é, em parte, a resposta.
...
- **Rodauth:** tecnicamente o mais robusto dos três — feature set vasto (2FA, account lockout, audit log, JWT, OmniAuth, WebAuthn), arquitetura desacoplada do model, DSL configurável. Rejeitado porque: (a) curva de aprendizado alta, DSL própria de Roda/Rodauth; (b) muito menos comum em vagas BR — o sinal "conheço Rodauth" tem ROI baixo no objetivo de gerar entrevistas; (c) acoplamento ao Sequel/Roda fora do ecossistema Rails idiomático cria atrito conceitual num app que é Rails canônico no resto.
